 ...
 */
export const getChallengeStatus = (userId: string): ChallengeStatus => {
 ...
 */
export const completeDailyChallenge = (userId: string): { newStreak: number } => {
  const key = `${CHALLENGE_KEY_PREFIX}${userId}`;
  const status = getChallengeStatus(userId);
