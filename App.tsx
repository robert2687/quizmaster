...
import { postScore, initLeaderboardSimulation } from './services/leaderboardService';
...
const App: React.FC = () => {
  ...
  // Handlers for showing different leaderboards
  const handleViewLeaderboard = () => setLeaderboardConfig({ title: t('leaderboardTitle') });
  const handleViewDailyChallengeLeaderboard = () => setLeaderboardConfig({
    title: t('dailyChallengeLeaderboardTitle'),
    topicFilter: dailyChallengeTopic,
  });
  const handleCloseLeaderboard = () => setLeaderboardConfig(null);

  const renderIdleContent = () => {
    return (
      <div className="w-full max-w-lg">
        <div className="mb-6 p-4 border-2 border-dashed border-purple-500/50 rounded-lg text-center bg-slate-800/50 shadow-inner">
          <div className="flex items-center justify-center gap-2">
            <TrophyIcon className="w-6 h-6 text-yellow-400"/>
            <h3 className="text-lg font-bold text-yellow-300">{t('dailyChallengeTitle')}</h3>
          </div>
          <p className="text-slate-200 my-2 text-xl font-semibold">"{dailyChallengeTopic}"</p>
          <div className="flex flex-col sm:flex-row gap-2 justify-center mt-3">
             <Button
                onClick={() => handleGenerateQuiz(dailyChallengeTopic, Difficulty.MEDIUM, true)}
                variant="secondary"
                className="!from-yellow-500 !to-orange-500 hover:!from-yellow-600 hover:!to-orange-600 focus:!ring-yellow-400"
              >
                {t('playChallengeButton')}
              </Button>
              <Button onClick={handleViewDailyChallengeLeaderboard}>
        ...
        <div className="flex items-center my-6">
          ...
          <span className="flex-shrink mx-4 text-slate-400 uppercase text-xs font-bold">Or</span>
          <div className="flex-grow border-t border-slate-700"></div>
        </div>
    ...
    if (leaderboardConfig) {
      return (
        <LeaderboardDisplay
          ...
          title={leaderboardConfig.title}
          topicFilter={leaderboardConfig.topicFilter}
        />
      );
    }
      ...
      <footer className="text-center py-4 mt-auto w-full max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
          <div className="flex-grow text-left">
             {currentUser && quizState !== QuizState.AUTH && !leaderboardConfig && (
